    ...
    <main class="relative w-full max-w-7xl mx-auto p-5">
        ...
        <form action="{{ route('activation.send', $id) }}" method="post" class="relative">
            ...
            <div class="relative ">
                ...
                <input type="email" required="required" autocomplete="yourmail@yourprovider.com" name="email"
                    id="message" cols="30" rows="10"
                    class=" mt-1 block w-full  max-w-md rounded-md border-2 shadow-sm  px-4 py-3 " />
                @error('email')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ __('Oops!') }}
                        </span>{{ $message }}</p>
                @enderror
            </div>
            <div class="relative ">
                <label for="name" class="block text-sm font-medium text-gray-700">{{ __('Code') }}*</label>
                <input name="code" required="required" autocomplete="XX-XX-XXXX" id="code"
                    value="{{ $code }}"
                    class=" mt-1 block w-full  max-w-md rounded-md border-2 shadow-sm  px-4 py-3 " />
                @error('code')
                    <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ __('Oops!') }}
