            ...
            <div class="relative">
                <div class="mt-5 md:col-span-2 md:mt-0">
                    <form action="{{ route('tags.store') }}" method="POST">
                        <img src="{{ public_url('./img/logo_withouttext.svg') }}" alt="Logo" class="float-right"
                            style="float:right" />
                        @csrf
                        @if ($tag['id'] != -1)
                            <a href="{{ $tag->getSightingURL() }}"
                                class="relative w-full flex justify-start items-center pb-14 my-5">
                                <img class="lg:w-[65%] w-[120%] lg:pl-0 pl-[11px] lg:-mr-52 mr-[-65px]"
                                    src="data:image/svg+xml;base64, {{ $tag['qr'] }}" />
                            </a>
                        @endif
                                ...
                                <div class="grid grid-cols-6 gap-6">
                                    ...
                                    <div
                                        ...
                                        <input type="text" id="name" name="name"
                                            value='{{ $tag['name'] ? $tag['name'] : old('name') }}' placeholder="Name"
                                            autocomplete="name"
                                    ...
                                    <div class="col-span-6 sm:col-span-3">
                                        ...
                                        <input type="text" id="code" name="code"
                                            value='{{ $tag['code'] ? $tag['code'] : old('code') }}' placeholder="Fas-ID"
                                            @if (!Auth::user()->isAdmin()) readonly="readonly"
                                    ...
                                    {{-- valid_unitl --}}
                                    <div class="col-span-6 " x-data="app()" x-init="[initDate(), getNoOfDays()]" x-cloak>

                                        <div class="relative">
                                            @if (Auth::user()->isAdmin())
                                                <label for="valid_until"
                                                    class="block text-sm font-medium text-gray-700">{{ __('Bis') }}</label>
                                                <input type="text" id="valid_until" name="valid_until" readonly
                                                    @if (Auth::user()->isAdmin()) x-on:click="showDatepicker = !showDatepicker"
                                                       x-model="datepickerValue"
                                                       x-on:keydown.escape="showDatepicker = false" @endif
                                                ...
                                                <div class="bg-white mt-12 rounded-lg shadow p-4 z-20 absolute top-0 left-0"
                                    ...
                                    @if (Auth::user()->isAdmin())
                                        <div class="col-span-6 sm:col-span-3">
                                            <label for="activationcode" class="block text-sm font-medium text-gray-700"><a
